...
function display(item: MedicationChecklistItem) {
  const brand = [...item.brands].filter((entry) => entry.showInsteadOfGeneric && entry.name.trim()).sort((left, right) => left.priority - right.priority)[0];
  return {
    name: brand?.name.trim() || item.genericName,
    brand,
